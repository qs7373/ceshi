...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QLabel, QLineEdit, QPushButton, QGridLayout
import pandas as pd
import requests
...
from PyQt5.QtWidgets import QWidget, QLabel, QVBoxLayout, QHBoxLayout, QComboBox, QFileDialog, QPushButton
...
system_fonts = []
selected_image = None
settings = {"size": "A4", "custom_size": "210x297", "font": "SimSun", "size_num": 12, "leading": 12, "tracking": 0}
card_settings = {"paper_size": "A4", "custom_size": "210x297",  "font": "SimSun","size": 24,"leading": 36,
                 "crop_margin": 10,
                 "auto_crop": True,
...
card_settings["paper_size"] = "A4"
card_settings["font"] = "SimSun"
card_settings["size"] = 20
card_settings["leading"] = 1.5
card_settings["tracking"]= 0
card_settings["crop_margin"] = 3
card_settings["vertical"] = False
...
class TemplateEditor(QWidget):
    def __init__(self):
        ...
        self.my_widget.my_signal.connect(self.my_slot)
...
def print_document(file_path, sheet_name, print_range):
    ...
    if file_type == ".xlsx":
        ...
        pdf_file = os.path.join(os.path.dirname(file_path), f"temp_{timestamp}.pdf")
        # 创建一个画布对象将内容打印到PDF文件
        ...
        for page in print_area:
            output_pdf.addPage(page)
        # 创建一个临时PDF文件来存储打印内容
        temp_file_name = "temp_" + str(int(time.time())) + ".pdf"
        pdf_file = os.path.join(os.path.dirname(file_path), temp_file_name)
        # 将输出PDF写入临时文件
        with open(pdf_file, "wb") as output:
            output_pdf.write(output)
        # 使用subprocess将PDF文件打印到默认打印机
        ...
        if print_range:
            ...
            print_area = lines
        # 创建一个临时文本文件来存储打印内容
        temp_file_name = "temp_" + str(int(time.time())) + ".txt"
        txt_file = os.path.join(os.path.dirname(file_path), temp_file_name)
        # 将打印区域写入临时文本文件
...
# 定义打印函数
def print_text(printer_name, text_to_print, header=None, footer=None, watermark=None):
    # 检查操作系统类型是否支持打印功能
    if get_os_type() == 'Unsupported':
    ...
    if sys.platform.startswith('win'):
        ...
        conn = win32print.OpenPrinter(printer_name)
    ...
    # 填充 printers 字典
    printers = {
        "Printer 1": {"device-uri": "uri1"},
        "Printer 2": {"device-uri": "uri2"},
        "Canon TS3400 series": {"device-uri": "uri3"},  # 将注释行移动到这里
    }
    ...
    if printer_name not in printers:
        raise ValueError(f"打印机 {printer_name} 不存在")
    ...
    # 打印文本
    if get_os_type() == 'Windows':
        # 如果是Windows，使用win32print库打印
        win32print.StartDocPrinter(handle, 1, ('test', None, "RAW"))
        win32print.WritePrinter(handle, text.encode())
        win32print.EndDocPrinter(handle)
        win32print.ClosePrinter(handle)
        subprocess.run(['print', '-d', printer_name], input=text_to_print.encode('utf-8'), check=True)
    elif get_os_type() == 'Mac':
        # 如果是Mac，使用osx-printer库打印
        handle.print_file(text, 'text/plain')
...
def add_page_numbers(text):
    ...
    # 检查文本的行数是否正确
    if len(lines) != num_pages * 60:
        lines += [''] * (num_pages * 60 - len(lines))
...
def find_printer(printer_name):
    ...
    for printer in printers:
        if printer.lower() == printer_name.lower():
            printer_uri = printers[printer]['device-uri']
            return printer_uri
    # 如果没有找到指定名称的打印机，抛出异常
if not target_printer is None:
...
def get_os_type():
    ...
    if system_type == 'Windows':
        ...
        raise Exception(f"Unsupported system type: {system_type}")


def get_printer_info(printer_uri, printer_type):
    if printer_type == 'usb':
        # 如果是USB打印机，返回打印机名称
        return printer_uri.split(':')
    elif printer_type == 'socket':
        # 如果是网络打印机，返回IP地址和端口号
        address = printer_uri.split(':')[1]
        port = printer_uri.split(':')[2]
        return address, port
    else:
        raise Exception(f"Unsupported printer type: {printer_type}")


def print_text(text, header=None, footer=None, watermark=None, printer_uri=None, printer_address=None):
    # 添加页眉
    if header:
        text = header + '\n' + text
    ...
    if printer_address:
        ...
        conn = cups.Connection()
        printers = conn.getPrinters()
        target_printer_name = "My Printer"  # 替换为目标打印机的名称
        target_printer = printers[target_printer_name]
        job_options = {
            'media': 'A4',
            'fit-to-page': True,
        }
        conn.printFile(target_printer_name, "-", job_options, text.encode('utf-8'))

    if platform.system() == 'Windows':
        # 如果是Windows系统，使用Win32 API连接打印机
        target_printer_name = "My Printer"  # 替换为目标打印机的名称
        printer_handle = win32print.OpenPrinter(target_printer_name)
        job_info = win32print.GetDefaultPrinter()
        job_options = {
            'DesiredAccess': win32print.JOB_ACCESS_ADMINISTER,
            'pDatatype': 'RAW',
        }
    ...
    if printer_uri:
        printer_type = printer_uri.split(':')[0]
        return get_printer_info(printer_uri, printer_type)
    else:
        return None


# 示例用法
printer_uri = "usb:my_printer"
printer_type = "usb"
printer_info = get_printer_info(printer_uri, printer_type)
print(printer_info)  # 输出：['usb', 'my_printer']

text_to_print = "This is a test print"
print_text(text_to_print, printer_uri=printer_uri)  # 打印文本到指定打印
...
def create_gui():
    ...
    if sys.platform == 'win32':
        ...
        # 创建 GUI 组件
        tk.Label(root, text="Remote Printer").grid(row=0, column=0)
        tk.Entry(root, textvariable=tk.StringVar(root)).grid(row=0, column=1)
        tk.Label(root, text="Port").grid(row=1, column=0)
        tk.Entry(root, textvariable=tk.StringVar(root)).grid(row=1, column=1)
        tk.Label(root, text="Printer").grid(row=2, column=0)
        printer_menu.grid(row=2, column=1)
        tk.Button(root, text="Print", command=lambda: print_file(printer_var.get())).grid(row=3, column=0, columnspan=2)
...
# 初始化远程打印机
addresses = {'printer1': ('192.168.0.100', 9999), 'printer2': ('192.168.1.99', 8888)}

printer_name = 'printer1' # 或者 'printer2'，或者其他打印机名称
ip_address, port = addresses[printer_name]

remote_printer = RemotePrinter(ip_address, port)
...
def save_settings():
        ...
        settings = {"custom_size": custom_size, "font": font, "size": size, "leading": leading, "tracking": tracking}
...
def connect_printer(printer):
    ...
    for printer_name in printers:
        if printer_name.lower() == target_printer_name.lower():
            target_printer = Printer(printer_name, printers[printer])
    # 连接蓝牙打印机
    subprocess.call(["sudo", "hciconfig", "hci0", "piscan"])
    socket = bluetooth.BluetoothSocket(bluetooth.RFCOMM)
    ...
    socket.send(b"\x1b\x52\x08")  # 设置字符编码为UTF-8
...
def save_settings():
    ...
    settings = {"orientation": orientation, "custom_size": custom_size, "font": font, "size": size, "leading": leading, "tracking": tracking, "batch_size": batch_size}
    with open("settings.pkl", "wb") as f:
        pickle.dump(settings, f)
